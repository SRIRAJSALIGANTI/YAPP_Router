...
//------------------------------------------------------------------------------
// SEQUENCE: router_exhaustive_coverage_vseq
// Target: Coordinates HBUS and YAPP for 100% functional coverage
//------------------------------------------------------------------------------
class router_exhaustive_coverage_vseq extends uvm_sequence;

  `uvm_object_utils(router_exhaustive_coverage_vseq)
  `uvm_declare_p_sequencer(router_virtual_sequencer)

  // Sub-sequence handles matching your library's naming conventions
  hbus_set_default_regs_seq    hbus_default; 
  hbus_get_yapp_regs_seq       hbus_read_back; // Added handle for Register Reads
  yapp_exhaustive_coverage_seq yapp_cov_pkt_stream;

  function new(string name="router_exhaustive_coverage_vseq");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    `uvm_info(get_type_name(), "Running Coordinated Functional Coverage Sweep...", UVM_LOW)

    // 1. Configure the DUT registers (Triggers HBUS Write coverage bins)
    `uvm_do_on(hbus_default, p_sequencer.hbus_seqr)

    // 2. Read back all configuration registers (Triggers HBUS Read coverage bins)
    `uvm_info(get_type_name(), "Executing HBUS Read-Back Sweep for Register Coverage...", UVM_LOW)
    `uvm_do_on(hbus_read_back, p_sequencer.hbus_seqr)

    // 3. Fire the custom YAPP loop with 0-delay transitions and max payload sweeps
    `uvm_do_on(yapp_cov_pkt_stream, p_sequencer.yapp_seqr)
...
endclass : router_exhaustive_coverage_vseq
